    ...
    @foreach ($objetos as $objeto)
        <div wire:key="objetoCed_{{ $objeto->img_id }}">
            ...
            @else
                ...
                @elseif(($edit=='1' and !method_exists('AbreModalObjetoEnCedula')))
                    No hay método de edición
                @endif
            @endif
        </div>
    @endforeach

    @if($objetos->count()=='0' and $edit=='1')
        <div class="margin:20px; text-align:center;">
            <center>
                <button class="btn" wire:click="AbreModalObjetoEnCedula('0','{{ $TipoDeObjeto }}')" style="margin-top:30px;">
                    <i class="bi bi-image" style="color:#87796d;">
                        {{ $TipoDeObjeto }}
                    </i>
                </button>
            </center>
